...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, emb_size, encoder_hidden_size, decoder_hidden_size, dropout=0.2, model_type="RNN"):
        ...
        if model_type=="RNN":
            ...
            self.recur = nn.RNN(emb_size,encoder_hidden_size,batch_first=True)
        else:
            self.recur = nn.LSTM(emb_size,encoder_hidden_size,batch_first = True)
    ...
    def forward(self, input):
        ...
        #input -> (b,seq)
        #embed = self.dropout(self.embed(input)) ->(batch, seq, emb)
        #rnn-input needs ->(b,seq,emb_input) if batch_first
        
        if self.model_type=="RNN":
            output,h_n = self.recur(self.dropout(self.embed(input)))
            h_n = self.tanh(self.linear2(self.relu(self.linear1(h_n))))
            hidden = h_n
        else:
            output,(h_n,c_n) = self.recur(self.dropout(self.embed(input)))
            h_n = self.tanh(self.linear2(self.relu(self.linear1(h_n))))
            #c_n = self.tanh(self.linear2(self.relu(self.linear1(c_n))))
            hidden = (h_n,c_n)
