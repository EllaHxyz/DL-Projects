...
gbp_attr = gbp.attribute(X_tensor,target=y_tensor)

guided_gradcam = GuidedGradCam(model, conv_module)
gradcam_attr = guided_gradcam.attribute(X_tensor,target=y_tensor)


#visualization
visualize_attr_maps('visualization/guided_backprop_captum.png', X, y, class_names, [gbp_attr], ['Guided Backprop'])

visualize_attr_maps('visualization/guided_gradcam_captum.png', X, y, class_names, [gradcam_attr], ['Guided GradCam'])
...
layer_gc_attr = layer_gc.attribute(X_tensor,target=y_tensor,relu_attributions=True)
print(layer_gc_attr.size())
#visualization
visualize_attr_maps('visualization/layer_vis_captum.png', X, y, class_names, [layer_gc_attr], ['Layer GradCam'])
...
layer_cond_attr_sum = layer_cond_attr.mean(axis=1, keepdim=True)
    #(size 5,128,55,55 ???) should be (5,1,55,55)?

#visualization
visualize_attr_maps('visualization/layer_cond_captum.png', X, y, class_names, [layer_cond_attr_sum], ['Layer Conductance'])
