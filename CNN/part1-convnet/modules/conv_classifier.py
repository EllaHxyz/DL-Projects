...
class ConvNet:
    ...
    def forward(self, x, y):
        ...
        #get loss,prob 
        loss_model = self.criterion
        probs,loss = loss_model.forward(x,y)
    ...
    def backward(self):
        ...
        #get initial dout from Loss fcn
        loss_model = self.criterion
        loss_model.backward()
        dout = loss_model.dx
        
        #reversed modules for backward gradient flow
        for module in reversed(self.modules):
            
            model=module        
            #backward to update dx,dw,db
            model.backward(dout)
            #update dout
            dout = model.dx
