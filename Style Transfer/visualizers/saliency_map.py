...
class SaliencyMap:
    def compute_saliency_maps(self, X, y, model):
        ...
        #1. forward - get raw score matrix for input X  -> (N,C)
        score_mat = model(X_var)
        ...
        #2. backward - sum of non-scalar tensor and apply backward, for dS/dX
        score.sum().backward()
        
        #4. get saliency value
        #max, max_index = torch.max(input, dim = the dim to reduce)
