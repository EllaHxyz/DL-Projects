...
def style_transfer(name, content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(200):
        ...
        #reduce lr for the last few epochs
       
        if t==decay_lr_at:
            for g in optimizer.param_groups:
                g['lr'] = decayed_lr
        
        #get current img layer
        current_img = feats[content_layer].clone()
        
        '''forward''' 
        #content loss
        Lc = content_loss(content_weight,current_img,content_target)
        #style loss
        Ls = style_loss(feats,style_layers,style_targets,style_weights)
        #total loss
        Lt = tv_loss(img_var, tv_weight)
        
        #Loss = content loss+style_loss+tv_loss
        loss =Lc+Ls+Lt
        #print(loss)  
        '''backward'''     
        #set zero optimizer grad
        optimizer.zero_grad() 
         
        #grad that min. loss
        loss.backward()
        #optimize input pixel
        optimizer.step()
