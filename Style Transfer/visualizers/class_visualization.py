...
#import sys 
#sys.path.append('/Users/Ella/Desktop/CSProjects/DL/assignment3')
...
class ClassVisualization:
    ...
    def create_class_visualization(self, target_y, class_names, model, dtype, **kwargs):
        ...
        for t in range(num_iterations):
            ...
            img_var.data.copy_(self.jitter(img_var.data, ox, oy))
            ...
            #Note:by default, gradients are accumulated in buffers (i.e, not overwritten) 
            #whenever .backward() is called.So we need to zero-out gradients for each batch iteration.
            
            #0.zero-out gradients
            model.zero_grad()
            
            #1. forward - get raw score of I
            score_list = model(img_var)
            s_i = score_list[:,target_y]
            #2. add regularization
            l2 = torch.sum(torch.pow(img_var.data,2))
            S_I = s_i - l2_reg*l2
            #3. backward - get backward gradients, including dS/dI
            S_I.backward()
            
            #4. optimize - update I by gradient ascent
            img_var.data += learning_rate*img_var.grad.data/torch.norm(img_var.grad.data)
